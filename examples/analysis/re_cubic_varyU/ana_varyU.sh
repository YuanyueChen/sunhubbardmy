       ...
       for u in ${uarray}; do
	    ...
	    echo " lq = $lq "
            ...
            if [ -f spsm_orb1_k.bin ]; then
                ...
                grep -e "  0.50000000E+00  0.50000000E+00  0.50000000E+00" spsm_orb1_k.bin |awk '{if(NR>nskipv) print $4}' nskipv=$nskip |sort -n > spsm_pi.tmp.tmp
                ...
                {if(NR==1 && abs($3)<abs($1)) print uv, 1-$3/$1, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1); \
                 else if(NR==1) print uv, 0, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1)}' uv=$u >> $WORKDIR/${pretag}_spsm_pi_r100.dat
                ...
                paste spsm_pi.tmp2 spsm_pidq.tmp2 |awk 'function abs(v) {return v<0?-v:v} \
                {if(NR==1 && abs($3)<abs($1)) print uv, 1-$3/$1, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1); \
                 else if(NR==1) print uv, 0, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1)}' uv=$u >> $WORKDIR/${pretag}_spsm_pi_r110.dat
                ...
                paste spsm_pi.tmp2 spsm_pidq.tmp2 |awk 'function abs(v) {return v<0?-v:v} \
                {if(NR==1 && abs($3)<abs($1)) print uv, 1-$3/$1, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1); \
                 else if(NR==1) print uv, 0, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1)}' uv=$u >> $WORKDIR/${pretag}_spsm_pi_r111.dat
                ...
                paste spsm_pi.tmp2 spsm_pidq.tmp2 |awk 'function abs(v) {return v<0?-v:v} \
                {if(NR==1 && abs($3)<abs($1)) print uv, 1-$3/$1, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1); \
                 else if(NR==1) print uv, 0, sqrt($4*$1*$4*$1+$2*$3*$2*$3)/($1*$1)}' uv=$u >> $WORKDIR/${pretag}_spsm_pi_r200.dat
		...
		awk -v lta1v=$lta1 -v lqta1v=$lqta1 -v nskipv=$nskip '{if(NR>lqta1v*nskipv) {if(NR%lqta1v>=lqta1v-lta1v+2 || NR%lqta1v==0) sum +=$1; if(NR%lqta1v==0) {printf( "%12.6f \n", sum/(lta1v-1)); sum=0}} }' spsm_ktau.bin |sort -n > chi.tmp
                ...
                awk 'function abs(v) {return v<0?-v:v} {for(i=1;i<=NF;i++) {sum[i] += $i; sumsq[i] += ($i)^2}} END {for (i=1;i<=NF;i++)  { printf( "%12.6f %12.6f \n", sum[i]/NR, sqrt(abs(sumsq[i]-sum[i]^2/NR)/NR) )} }'\
                chi.tmp1 > chi.tmp2
                awk '{if(NR==1) print uv, $0}' uv=$u chi.tmp2 >> $WORKDIR/${pretag}_chi.dat
