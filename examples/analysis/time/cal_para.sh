#!/bin/bash
plquarray=$( echo "0.00")
uarray=$(awk 'BEGIN{for(i=1.00;i<=1.001;i+=1.00) printf("%6.2f",i)}')
#uarray=$( echo "2.00 3.00 4.00 5.00")
varray=$(awk 'BEGIN{for(i=0.00;i<=0.001;i+=1.00) printf("%6.2f",i)}')
#varray=$( echo "2.00 3.00 4.00 5.00")
betaarray=$( echo "1.000")
Larray=$(awk 'BEGIN{for(i=60.00;i<=60.001;i+=4.00) printf("%6.2f",i)}')
#Larray=$( echo "38")
#Larray=$( echo "26 30 34 38 42")
alpha=0.00
theta=0.00
nflrarray=$( echo "2")
nuarray=$( echo "0.00")
codearray=$( echo "square_ft_check_submatrix_timing square_ft_check_delay_timing")
xmag=0.0
flux_x=0.0
flux_y=0.0
rndness=0.00001
