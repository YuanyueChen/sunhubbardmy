  ...
  ! subrouinte s_diag_zH_x_d( ndim, Amat, dvecr, Bmat )
  ...
  ! subroutine s_v_invd_uH( ndim, vmat, dvec, umat, zmat )
  ...
  ! subroutine s_v_d_uH( ndim, vmat, dvec, umat, zmat )
  ! subroutine s_vH_d_uH( ndim, vmat, dvec, umat, zmat ) ! product of conjugate of a complex matrix, a real diagonal matrix and conjugate of another complex matrix
  ...
  ! subroutine s_invdiag_d_x_zr( ndim, dvec, Amat, Bmat ) ! product of inverse a real diagonal and a complex upper triangluar matrix
  ! subroutine s_invdiag_d_x_z( ndim, dvec, Amat, Bmat ) ! product of inverse a real diagonal and a complex matrix
  ! subroutine s_invdiag_z_x_d( ndim, Amat, dvec, Bmat )
  ! subroutine s_invdiag_zH_x_d( ndim, Amat, dvec, Bmat ) ! product of a conjugate of a complex matrix and inverse of a real diagonal
  ! subroutine s_zgeQR(rdim, cdim, amat, qmat, rmat ) ! QR decomposition
  ! subroutine s_zgeQDT(ndim, amat, qmat, dvec, tmat) ! QR decomposition to get QDT
  ! subroutine s_zgeqp3QDT(ndim, amat, qmat, dvec, tmat) ! pivoted QR decomposition to get QDT
  ! subroutine s_zgeQR_logdetQ(rdim, cdim, amat, qmat, rmat, zlogdet ) ! QR decomposition, also return logdet(Q)
  ! subroutine s_zgeQR_pqmc(rdim, cdim, amat, qmat, rmat, rlogdet) ! QR or LQ decomposition for PQMC, rmat with positive det, also return |det(rmat)|
  ! subroutine s_zgeQRPT_pqmc(rdim, cdim, amat, qmat, rmat, rlogdet) ! QR*PT decomposition for PQMC, doest work well
  ...
  ! subroutine s_pinv_logdet_svd_z(ndim, eps0, amat, zlogdet ) ! pseudo inverse of a complex matrix, using UDV decomposition, also return logdet of this matrix
...
!$OMP END PARALLEL
  ...
  subroutine s_diag_zH_x_d( ndim, Amat, dvec, Bmat )
  !! matrix product of conjugate of a complex matrix Amat and a real diagonal matrix (diagonal elements are stored in a vector dvec)
#ifdef _OPENMP
    USE OMP_LIB
#endif
    use constants, only: dp, zero
...
!$OMP DO
    do i = 1, ndim
        do j = 1, ndim
            Bmat(j,i) = dcmplx( dvec(i), zero ) * dconjg(Amat(i,j))
        end do
    end do
!$OMP END DO
!$OMP END PARALLEL

  end subroutine s_diag_zH_x_d
...
!$OMP END DO
!$OMP END PARALLEL
    call zgemm('n','c',ndim,ndim,ndim,cone,v_invd,ndim,umat,ndim,czero,zmat,ndim)
    deallocate( v_invd )
  end subroutine s_v_invd_uH
...
!$OMP END DO
!$OMP END PARALLEL
    call zgemm('n','c',ndim,ndim,ndim,cone,v_d,ndim,umat,ndim,czero,zmat,ndim)
    deallocate( v_d )
  end subroutine s_v_d_uH

  subroutine s_vH_d_uH( ndim, vmat, dvec, umat, zmat )
  !! matrix product of conjugate of a complex matrix vmat, a real diagonal matrix (diagonal elements are stored in a vector dvecr)
  !! and conjugate of another complex matrix umat
#ifdef _OPENMP
    USE OMP_LIB
#endif
    use constants, only: dp, czero, cone
...
!$OMP DO
    do j = 1, ndim
        do i = 1, ndim
            v_d(i,j) = dconjg(vmat(j,i)) * dvec(j)
        end do
    end do
!$OMP END DO
!$OMP END PARALLEL
...
!$OMP END PARALLEL
  ...
  subroutine s_zgeQRPT(rdim, cdim, amat, qmat, rmat, jpvt)
    ...
    complex(dp), dimension(rdim, min(rdim,cdim) ), intent(out) :: qmat

    ! local
    integer  :: i, j, ierror, lwork, mindim
...
!$OMP END PARALLEL
    ...
    allocate(work(1))
    ! perform lwork query
    call zungqr(rdim, mindim, mindim, qmat, mindim, tau, work, lwork, ierror)
    lwork = nint(dble(work(1)))
...
!$OMP END PARALLEL
  ...
  subroutine s_invdiag_d_x_z( ndim, dvec, Amat, Bmat )
#ifdef _OPENMP
    USE OMP_LIB
#endif
    use constants, only: dp, czero, zero
...
!$OMP END DO
!$OMP END PARALLEL
  end subroutine s_invdiag_d_x_z
...
!$OMP DO
    do i = 1, ndim
        do j = 1, ndim
            Bmat(j,i) =  Amat(j,i) / dcmplx( dvec(i), zero )
        end do
    end do
!$OMP END DO
!$OMP END PARALLEL
  end subroutine s_invdiag_z_x_d

  subroutine s_invdiag_zH_x_d( ndim, Amat, dvec, Bmat )
#ifdef _OPENMP
    USE OMP_LIB
#endif
    use constants, only: dp, czero, zero
...
!$OMP DO
    do i = 1, ndim
        do j = 1, ndim
            Bmat(j,i) =  dconjg( Amat(i,j) ) / dcmplx( dvec(i), zero )
        end do
    end do
!$OMP END DO
!$OMP END PARALLEL
  end subroutine s_invdiag_zH_x_d
...
!$OMP END PARALLEL
  ...
  subroutine s_zgeQDT(ndim, amat, qmat, dvec, tmat) ! QR decomposition to get QDT
    ...
    USE OMP_LIB
      ...
      integer, intent(in) :: ndim
      complex(dp), dimension(ndim,ndim), intent(in) :: amat
      complex(dp), dimension(ndim,ndim), intent(out) :: qmat
      real(dp), dimension(ndim), intent(out) :: dvec
      complex(dp), dimension(ndim, ndim), intent(out) :: tmat

      ! local
      integer :: i
      complex(dp), dimension(:,:), allocatable :: rmat
      allocate( rmat(ndim,ndim) )
      call s_zgeQR(ndim, ndim, amat, qmat, rmat)
!$OMP PARALLEL &
!$OMP PRIVATE ( i )
!$OMP DO
      do i = 1, ndim
          dvec(i) = dble(rmat(i,i))
      end do
!$OMP END DO
!$OMP END PARALLEL
      call s_invdiag_d_x_zr( ndim, dvec, rmat, tmat)
      deallocate( rmat )
  end subroutine s_zgeQDT
  ...
  subroutine s_zgeqp3QDT(ndim, amat, qmat, dvec, tmat) ! pivoted QR decomposition to get QDT
      ...
      allocate( rmat(ndim,ndim) )
      allocate( jpvt(ndim) )
      call s_zgeQRPT(ndim, ndim, amat, qmat, rmat, jpvt)
      do i = 1, ndim
          dvec(i) = dble(rmat(i,i))
      end do
      call s_invdiag_d_x_zr( ndim, dvec, rmat, tmat)
      call s_zmp(ndim,ndim,tmat,jpvt)
      deallocate( jpvt )
      deallocate( rmat )
  end subroutine s_zgeqp3QDT

  subroutine s_zgeQRPT_pqmc(rdim, cdim, amat, qmat, rmat, rlogdet)
  ! perform QR decomposition with columns pivoting (AP=QR) for PQMC, where we make R*PT to have positive determinant
  ! Note P(transpose of jpvt in the following) is not returned, as we only want to use qmat
  ! if rdim > cdim, we perform QR on amat
  ! if rdim < cdim, we perform LQ on amat, which can be done first with QR on amat^T, and then transpose back
#ifdef _OPENMP
    USE OMP_LIB
#endif
    use constants, only : dp, zero, czero, one, cone
    implicit none
    integer, intent(in) :: rdim, cdim
    complex(dp), dimension(rdim,cdim), intent(in) :: amat
    complex(dp), dimension(rdim,cdim), intent(out) :: qmat
    complex(dp), dimension(min(rdim,cdim),min(rdim,cdim)), intent(out) :: rmat
    real(dp), intent(out) :: rlogdet

    ! local
    integer  :: i, j, ierror, lwork, mindim, ndim1, ndim2, isgn
    integer, dimension(:), allocatable :: jpvt
    real(dp), dimension(:), allocatable :: rwork
    real(dp) :: rtmp
    complex(dp), dimension(:,:), allocatable :: amattmp, qmattmp, rmattmp
    complex(dp), dimension(:), allocatable :: work
    complex(dp), dimension(:), allocatable :: tau

    integer :: npt, k, pv, itemp

    write(*,*) " WARNING: in s_zgeQRPT_pqmc, pass test, but does show any better performance than s_zgeQR_pqmc ! "
    !write(*,*) " WARNING: in s_zgeQRPT_pqmc, jpvt is changed and will not be returned ! "

    ! in the following, keep ndim1 >= ndim2
    if( rdim >= cdim ) then
        mindim = cdim
        ndim1 = rdim
        ndim2 = cdim
        allocate( amattmp(ndim1, ndim2) )
        amattmp = amat
    else
        mindim = rdim
        ndim1 = cdim
...
!$OMP DO
        do j = 1, ndim1
            do i = 1, ndim2
                amattmp(j, i) = amat(i, j)
            end do
        end do
!$OMP END DO
!$OMP END PARALLEL
    end if
    allocate( qmattmp(ndim1, ndim2) )
    allocate( rmattmp(ndim2, ndim2) )
    allocate( jpvt(ndim2) )
    !!!allocate( jpv(ndim2) )

    !! perform QR factorization on amattmp
    lwork=-1
    allocate( tau(mindim) )
    allocate( rwork(2*cdim) )
    allocate( work(1) )
    ! perform lwork query
    jpvt(:) = 0
    call zgeqp3(ndim1, ndim2, amattmp, ndim1, jpvt, tau, work, lwork, rwork, ierror)
    lwork = nint(dble(work(1)))
    !write(*,*) 'in zgeqp3, lwork =', lwork
    deallocate(work)
    ...
    allocate(work(lwork),stat=ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQRPT_pqmc, the place above zgeqp3 : can not allocate enough memory'
        stop
    endif
    jpvt(:) = 0
    call zgeqp3(ndim1, ndim2, amattmp, ndim1, jpvt, tau, work, lwork, rwork, ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQRPT_pqmc : error in lapack subroutine zgeqp3'
        stop
    endif
    deallocate(work)
    deallocate(rwork)
...
!$OMP PARALLEL &
!$OMP PRIVATE ( i, rtmp )
!$OMP DO REDUCTION ( + : isgn, rlogdet )
    do i = 1, mindim
        rtmp = dble(amattmp(i,i))
        !if( ( rtmp<0.d0 .and. jpvt(i)==i ) .or. ( rtmp>0.d0.and.jpvt(i).ne.i )  ) then
        if( rtmp < 0.d0 ) then
            isgn = isgn + 1
            rlogdet = rlogdet + log( -rtmp )
        else
            rlogdet = rlogdet + log(  rtmp )
        end if
    end do
...
!$OMP END PARALLEL
    ...
    ! write(50,'(a,18i4)') ' jpvt^T = ', jpv
    ! write(50,'(a,i8)') ' number of permutations = ', i
    ! write(50,'(a)') ' '
    !! Note jpvt now is changed
    if ( mod(i,2) .eq. 1) isgn = isgn + 1
...
!$OMP END PARALLEL
    ...
    allocate(work(1))
    ! perform lwork query
    call zungqr(ndim1, mindim, mindim, qmattmp, ndim1, tau, work, lwork, ierror)
    lwork = nint(dble(work(1)))
    !write(*,*) 'in zungqr, lwork =', lwork
    deallocate(work)
    ...
    allocate(work(lwork), stat=ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQRPT_pqmc, the place above zungqr : can not allocate enough memory'
        stop
    endif
    call zungqr(ndim1, mindim, mindim, qmattmp, ndim1, tau, work, lwork, ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQRPT_pqmc : error in lapack subroutine zungqr'
        stop
    endif
...
!$OMP DO
        do j = 1, ndim2
            do i = 1, ndim2
                qmat(j, i) = qmattmp(i, j)
                rmat(j, i) = rmattmp(i, j)
            end do
            do i = ndim2+1, ndim1
                qmat(j, i) = qmattmp(i, j)
            end do
...
!$OMP END PARALLEL
  ...
  end subroutine s_zgeQRPT_pqmc

  subroutine s_zgeQR_pqmc(rdim, cdim, amat, qmat, rmat, rlogdet)
  ! perform QR decomposition for PQMC, where we make R to have positive determinant
  ! if rdim > cdim, we perform QR on amat
  ! if rdim < cdim, we perform LQ on amat, which can be done first with QR on amat^T, and then transpose back
#ifdef _OPENMP
    USE OMP_LIB
...
!$OMP END PARALLEL
    ...
    allocate( work(1) )
    ! perform lwork query
    call zgeqrf(ndim1, ndim2, amattmp, ndim1, tau, work, lwork, ierror)
    lwork = nint(dble(work(1)))
    !write(*,*) 'in zgeqrf, lwork =', lwork
    deallocate(work)
    ...
    allocate(work(lwork),stat=ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQR_pqmc, the place above zgeqrf : can not allocate enough memory'
        stop
    endif
    call zgeqrf(ndim1, ndim2, amattmp, ndim1, tau, work, lwork, ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQR_pqmc : error in lapack subroutine zgeqrf'
        stop
    endif
    deallocate(work)
...
!$OMP END PARALLEL
    ...
    allocate(work(lwork), stat=ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQR_pqmc, the place above zungqr : can not allocate enough memory'
        stop
    endif
    call zungqr(ndim1, mindim, mindim, qmattmp, ndim1, tau, work, lwork, ierror)
    if ( ierror /= 0 ) then
        write(*,'(a)') 'error in s_zgeQR_pqmc : error in lapack subroutine zungqr'
        stop
    endif
    if( mod(isgn,2) .eq. 1 ) then
        qmattmp(:,1) = - qmattmp(:,1) 
...
!$OMP END PARALLEL
  ...
  subroutine s_pinv_logdet_svd_z(ndim, eps0, amat, zlogdet )
      ! pseudo inverse
#ifdef _OPENMP
    USE OMP_LIB
#endif
      ...
      integer :: i
      complex(dp) :: ztmp
      complex(dp), dimension(:,:), allocatable :: vmat, umat
      real(dp), dimension(:), allocatable :: svec
      allocate( umat(ndim,ndim), vmat(ndim,ndim) )
      allocate( svec(ndim) )
      call s_svd_zg(ndim, ndim, ndim, amat, umat, svec, vmat) ! UDV decomposition of a complex matrix
      zlogdet = czero
!$OMP PARALLEL &
!$OMP PRIVATE ( i )
!$OMP DO REDUCTION ( + : zlogdet )
      do i = 1, ndim
          if( svec(i) > eps0 ) then
              ...
              write(50,'(a,i6,a,e24.12)') 'warning: svec(', i, ') = ', svec(i)
              svec(i) = 1.d0/eps0
          end if
          zlogdet = zlogdet + dlog( svec(i) )
      end do
!$OMP END DO
!$OMP END PARALLEL
      call s_vH_d_uH( ndim, vmat, svec, umat, amat ) ! V^H * Sigma * U^H
      call s_logdet_z(ndim, umat, ztmp)
      zlogdet = zlogdet + ztmp
      call s_logdet_z(ndim, vmat, ztmp)
      zlogdet = zlogdet + ztmp
      deallocate( svec )
      deallocate( vmat, umat )
  end subroutine s_pinv_logdet_svd_z
