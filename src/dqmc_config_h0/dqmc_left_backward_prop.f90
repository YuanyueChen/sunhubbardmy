...
!$OMP DO
      do i = 1,latt%nn_lf
         ...
         do j = 1, n2
            v1(j) = this%urtm1(ist,1,1)*gmat%orb1(i1,j) + this%urtm1(ist,1,2)*gmat%orb1(i2,j) 
            v2(j) = this%urtm1(ist,2,1)*gmat%orb1(i1,j) + this%urtm1(ist,2,2)*gmat%orb1(i2,j) 
         enddo
         do j = 1, n2
            gmat%orb1(i1,j) = v1(j)
            gmat%orb1(i2,j) = v2(j)
...
#ENDIF
  ...
  if (rt.gt.zero) then
      call zgemm('n','n',ndim,n2,ndim,cone,this%urtm1,ndim,gmat%orb1,ndim,czero,Atmp%orb1,ndim)
      gmat = Atmp
  endif
#ENDIF
#IFDEF TIMING
