subroutine dqmc_left_forward_prop_hc(this, nf, gmat)
! B(t+dt,t)^+ *gmat
! for H0 part, we usually consider hermitian system, 
! therefore B^+ = B
! but for non-hermitian system, we should use B^+ directly,
...
!$OMP DO
      do i = 1,latt%nn_lf
         ...
         do j = 1, n2
            v1(j) = this%urt(ist,1,1) * gmat%blk1(i1,j) + this%urt(ist,1,2) * gmat%blk1(i2,j) 
            v2(j) = this%urt(ist,2,1) * gmat%blk1(i1,j) + this%urt(ist,2,2) * gmat%blk1(i2,j) 
         enddo
         do j = 1, n2
            gmat%blk1(i1,j) = v1(j)
            gmat%blk1(i2,j) = v2(j)
...
#ENDIF
  ...
  do j = 1, n2
      do i = 1, lq
          gmat%blk1(i,    j) = gmat%blk1(i,    j) * this%exph0k(i)
      end do
  end do
...
#ENDIF
  ...
  if (rt.gt.zero) then
      call zgemm('n','n',ndim,n2,ndim,cone,this%urt,ndim,gmat%blk1,ndim,czero,Atmp%blk1,ndim)
      gmat = Atmp
  endif
#ENDIF
#IFDEF TIMING
