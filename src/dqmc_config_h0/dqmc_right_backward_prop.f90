...
#ENDIF
  ...
  do i = 1, lq
      ...
      i2 = i + lq
      ! (D^-1)^+ (U^+ G^+)
      v1(:) = dconjg( this%exph0kinv(1,1,i) )*gtmpC(i1,:) + dconjg( this%exph0kinv(2,1,i) )*gtmpC(i2,:)
      v2(:) = dconjg( this%exph0kinv(1,2,i) )*gtmpC(i1,:) + dconjg( this%exph0kinv(2,2,i) )*gtmpC(i2,:)
      gtmpC(i1,:) = v1(:)
      gtmpC(i2,:) = v2(:)
  end do
