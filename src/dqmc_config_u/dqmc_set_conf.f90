  subroutine dqmc_set_conf( this, lq, ltrot, nu, lproju, alpha, theta, u, dtau )
    ...
    do is = 1, this%lcomp
      if (lproju ) then
          ...
          this%bmat%orb1(is) = exp( dcmplx(0.d0, pi*dble(2*is)/dble(this%lcomp)) )
          this%bmat_inv%orb1(is) = exp( dcmplx(0.d0,-pi*dble(2*is)/dble(this%lcomp)) )
      else
          if( u .ge. 0.d0 ) then
              this%phase(is) = exp( dcmplx(0.d0, this%alpha*(-etal(is))*0.5d0*(dble(nflr)+nu)) )
              this%bmat%orb1(is) =      exp( dcmplx(0.d0, this%alpha*etal(is)) )
              this%bmat_inv%orb1(is) =  exp( dcmplx(0.d0,-this%alpha*etal(is)) )
          else
              this%phase(is) = exp( dcmplx(this%alpha*(-etal(is))*0.5d0*(dble(nflr)+nu), 0.d0) )
              this%bmat%orb1(is) =      exp( dcmplx( this%alpha*etal(is), 0.d0) )
              this%bmat_inv%orb1(is) =  exp( dcmplx(-this%alpha*etal(is), 0.d0) )
          end if
      end if
#IFDEF PLEVEL2
      if( irank == 0 ) then
          write(fout,'(a)') " in uconf%set_conf "
          write(fout,'(a,i3,a)') "bmat%orb1(",is," ) = "
          write(fout,'(2f9.5)') this%bmat%orb1(is)
          write(fout,'(a)') " "
          write(fout,'(a,i3,a)') "bmat_inv%orb1(",is," ) = "
          write(fout,'(2f9.5)') this%bmat_inv%orb1(is)
          write(fout,'(a)') " "
      end if
#ENDIF
    end do
    ...
    do iflip = 1, this%lcomp-1
      ...
      if(lproju) then
          ...
          this%delta_bmat%orb1(iflip,is) = exp( dcmplx(0.d0, pi*dble(2*isp-2*is)/dble(this%lcomp)) ) - cone
      else
          if( u .ge. 0.d0 ) then
              this%phase_ratio(iflip,is) = exp( dcmplx(0.d0, this%alpha*(etal(is)-etal(isp))*0.5d0*(dble(nflr)+nu)) )
              this%delta_bmat%orb1(iflip,is) = exp( dcmplx(0.d0, this%alpha*(etal(isp)-etal(is))) ) - cone
          else
              this%phase_ratio(iflip,is) = exp( dcmplx(this%alpha*(etal(is)-etal(isp))*0.5d0*(dble(nflr)+nu),0.d0) )
              this%delta_bmat%orb1(iflip,is) = exp( dcmplx(this%alpha*(etal(isp)-etal(is)),0.d0) ) - cone
          end if
      end if
#IFDEF PLEVEL2
      if( irank == 0 ) then
          write(fout,'(a)') " "
          write(fout,'(a,i3,a,i3,a)') "delta_bmat%orb1(",iflip," ,",is," ) = "
          write(fout,'(2f9.5)') this%delta_bmat%orb1(iflip,is)
          write(fout,'(a)') " "
