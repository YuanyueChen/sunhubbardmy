...
#ENDIF
  ...
  call mpi_reduce( zn_bin, zn, 1, mpi_complex16, mpi_sum, 0, mpi_comm_world, ierr )
  ...
  if( irank .eq. 0 ) then
      ...
      zcpcm = zcpcm / znorm
      ftag = "cpcm"
      call fourier_trans_eqt(zcpcm, ftag)

      zspsm = zspsm / znorm
      ftag = "spsm"
      call fourier_trans_eqt(zspsm, ftag)

      znn = znn / znorm
      ftag = "nn"
      call fourier_trans_eqt(znn, ftag)
