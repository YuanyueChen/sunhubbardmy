...
module r_cubic_lattice   !in the rombohedral graphite(3 layers as a unit cell)
   use constants, only: dp
   use latt3d_class
  ...
  subroutine setup_r_cubic_sub(this, l1, l2, l3, a1_p, a2_p, a3_p)
    ...
    this%nsub = 6
    this%nsites = l1*l2*l3*6
    !this%z_plq = 18              ! sites contained in one cell?, 可重复计算. 
    this%nn_nf = 4
    this%nn_lf = l1*l2*l3*3 
    allocate( this%cord( this%nsites,3) )
    allocate( this%rcord( this%nsites,3) )
    allocate( this%nnlf_list(this%nn_lf) )
    allocate( this%list( this%nsites, 2) )
    allocate( this%invlist(l1, l2, l3,this%nsub))
    ...
    do i3 = 1, this%l3
        do i1 = 1, this%l1
            do i2 = 1, this%l2
                do isub = 1, this%nsub
                    ind = ind + 1
                    this%list( ind, 1) = (i3-1)*(l1*l2)+(i1-1)*l2+i2
                    this%list( ind, 2) = isub              ! sublattice
                    this%cord( ind, 1) = dble(i1)
                    this%cord( ind, 2) = dble(i2) 
                    this%cord( ind, 3) = dble(i3) 
                    this%invlist(i1,i2,i3,isub) = ind
                    if ( isub == 1) then ! the first layer A site
                        this%rcord( ind, 1) = dble(i1)
                        this%rcord( ind, 2) = dble(i2)
                        this%rcord( ind, 3) = dble(i3)
                    else if ( isub == 2 ) then ! the first layer B site
                        this%rcord( ind, 1) = dble(i1) + 1.d0/3.d0
                        this%rcord( ind, 2) = dble(i2) + 1.d0/3.d0
                        this%rcord( ind, 3) = dble(i3)
                    else if ( isub == 3 ) then ! the second layer A site
                        this%rcord( ind, 1) = dble(i1) + 1.d0/3.d0
                        this%rcord( ind, 2) = dble(i2) + 1.d0/3.d0
                        this%rcord( ind, 3) = dble(i3) + 1.d0/3.d0
                    else if ( isub == 4 ) then ! the second layer B site
                        this%rcord( ind, 1) = dble(i1) + 2.d0/3.d0
                        this%rcord( ind, 2) = dble(i2) + 2.d0/3.d0
                        this%rcord( ind, 3) = dble(i3) + 1.d0/3.d0
                    else if ( isub == 5 ) then ! the third layer A site
                        this%rcord( ind, 1) = dble(i1) + 2.d0/3.d0
                        this%rcord( ind, 2) = dble(i2) + 2.d0/3.d0
                        this%rcord( ind, 3) = dble(i3) + 2.d0/3.d0 
                    else if ( isub == 6 ) then ! the third layer B site
                        this%rcord( ind, 1) = dble(i1) + 1.d0
                        this%rcord( ind, 2) = dble(i2) + 1.d0
                        this%rcord( ind, 3) = dble(i3) + 2.d0/3.d0
                    end if
                end do
            end do
...
! set nearest neighbor
    ...
    !     this%nnvecr(:,2,i) = (/ -2.d0/3.d0, 1.d0/3.d0, 0.d0/)   
    !     this%nnvecr(:,3,i) = (/ 1.d0/3.d0, -2.d0/3.d0, 0.d0/)   
    !     this%nnvecr(:,4,i) = (/ 0.d0, 0.d0, -1.d0/3.d0/)   

    !     this%nnveci(:, 1, i) = (/ 0, 0, 0/) 
    !     this%nnveci(:, 2, i) = (/ -1, 0, 0/)
    !     this%nnveci(:, 3, i) = (/0, -1, 0/)
    !     this%nnveci(:, 4, i) = (/ 0, 0, 1/)
    ! end do    
    do i = 1, this%nsites
        if (( mod(i,6) == 3) .or. ( mod(i,6) == 5)) then !A2 or A3
            this%nnlist(i,1) = i+1
            this%nnlist(i,2) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1)-1, l1)-1)*l2 + npbc(this%cord(i,2), l2)-1)*this%nsub + mod(i,6)+1
            this%nnlist(i,3) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1),   l1)-1)*l2 + npbc(this%cord(i,2)-1, l2)-1)*this%nsub + mod(i,6)+1
            this%nnlist(i,4) = i-1
        else if ( mod(i,6) == 1) then !A1
            this%nnlist(i,1) = i+1
            this%nnlist(i,2) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1)-1, l1)-1)*l2 + npbc(this%cord(i,2), l2)-1)*this%nsub + mod(i,6)+1
            this%nnlist(i,3) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1),   l1)-1)*l2 + npbc(this%cord(i,2)-1, l2)-1)*this%nsub + mod(i,6)+1
            this%nnlist(i,4) = ((npbc(this%cord(i,3)-1, l3 )-1)*(l1*l2) + (npbc(this%cord(i,1)-1, l1)-1)*l2 + npbc(this%cord(i,2)-1, l2)-1)*this%nsub + mod(i,6)+1
        else if (( mod(i,6) == 2) .or. ( mod(i,6) == 4)) then !B1 or B2
            this%nnlist(i,1) = i-1
            this%nnlist(i,2) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1)+1, l1)-1)*l2 + npbc(this%cord(i,2), l2)-1)*this%nsub + mod(i,6)-1
            this%nnlist(i,3) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1),   l1)-1)*l2 + npbc(this%cord(i,2)+1, l2)-1)*this%nsub + mod(i,6)-1
            this%nnlist(i,4) = i+1
        else if ( mod(i,6) == 0) then !B3
            this%nnlist(i,1) = i-1
            this%nnlist(i,2) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1)+1, l1)-1)*l2 + npbc(this%cord(i,2), l2)-1)*this%nsub + 5
            this%nnlist(i,3) = ((npbc(this%cord(i,3)  , l3 )-1)*(l1*l2) + (npbc(this%cord(i,1),   l1)-1)*l2 + npbc(this%cord(i,2)+1, l2)-1)*this%nsub + 5
            this%nnlist(i,4) = ((npbc(this%cord(i,3)+1, l3 )-1)*(l1*l2) + (npbc(this%cord(i,1)+1, l1)-1)*l2 + npbc(this%cord(i,2)+1, l2)-1)*this%nsub + 1
        end if
    end do
    
    end subroutine setup_r_cubic_sub
end module r_cubic_lattice
