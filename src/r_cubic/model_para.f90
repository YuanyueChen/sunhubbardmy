module model_para
  ...
  real(dp), save :: rtd   ! nearest neighbor hopping in diffrent layers for R-CUBIC lattice
...
#ENDIF
  ...
  subroutine make_tables
...
#elif defined(R_CUBIC)
    namelist /model_para/ la, lb, lc, beta, dtau, rt, rtd, t2, t3, nu, mu, rhub, rv, rhub_plq, alpha, theta, nflr, lprojplqu, lprojv, lproju, xmag, flux_x, flux_y, dimer, rndness
#elif defined(CHAIN)
    ...
    ! default parameters
    la   = 2
#if (defined(HONEYCOMB) || defined(SQUARE) || defined(CUBIC) || defined(R_CUBIC))
    lb   = 2
#endif
...
#endif
#if defined(R_CUBIC)
    call mpi_bcast( rtd,         1, mpi_real8,    0, mpi_comm_world, ierr )
#endif
